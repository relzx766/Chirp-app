  ...
  <div>
    <el-row v-if="value.type===chirperTypeEnums.FORWARD"
      ...
      <el-col :offset=2 :span="21" class="text-dark-emphasis">
        <i class="el-icon-connection fw-bold" style="margin-left: -14px"/><span
          style="margin-left: 14px">{{ value.nickname }}&nbsp;已转发</span>
      </el-col>
    </el-row>
    <chirper-card v-if="value.type===chirperTypeEnums.FORWARD" :chirper="value.referenced"/>
    <chirper-card v-else-if="value.type===chirperTypeEnums.QUOTE||value.type===chirperTypeEnums.REPLY"
                  :barVisible="false" :chirper="value"
