...
export default {
    ...
    mutations: {
        [chatMutations.ADD_CHAT_RECORD](state, {chat, isNew}) {
            ...
            if (isNew) {
                ...
                state.record[chat.conversationId].date = new Date(chat.createTime).getTime();
            } else {
        ...
        [chatMutations.INCREASE_CONVERSATION_UNREAD](state, {conversation, unread}) {
        ...
        [chatMutations.MODIFY_TEMP_CHAT](state, {chat}) {
            if (chat.tempId && state.record[chat.conversationId] && state.record[chat.conversationId].messages) {
                let index = state.record[chat.conversationId].messages.findIndex(message => {
        ...
        [chatMutations.DEL_CHAT_RECORD](state, {conversation, id}) {
            if (state.record[conversation].messages) {
                let index = state.record[conversation].messages.findIndex(message => message.id === id);
                if (index !== -1) {
    ...
    actions: {
        [chatActions.INIT_CHAT]({commit, dispatch, state, rootState}) {
            return getChatIndexPage().then(res => {
                if (res.code === 200) {
                    return dispatch(chatActions.ADD_NEW_CHAT_RECORD, {chats: res.data.record});
                } else {
                    throw new Error(res.message);
                }
            }).then(() => {
                let conversations = Object.values(state.record).map(r => r.conversation);
        ...
        [chatActions.ADD_NEW_CHAT_RECORD]({commit, dispatch, state, rootState}, {chats, isNew = false}) {
            let unLoadPublicUserIds = [];
            let participants = new Set();
            chats.forEach(chat => {
                let other = getOtherParticipantID(chat.conversationId, rootState.user.id);
            ...
            //加载所有参与者的信息
            return dispatch(`user/${userActions.FETCH_USER}`, {userIds: participants}, {root: true}).then(() => {
                return dispatch(`e2ee/${e2eeActions.FETCH_PUBLIC_KEY}`, {userIds: unLoadPublicUserIds}, {root: true}).then(async () => {
                    let count = 0;
                    let unreadMap = {};
                    let unread = 0;
                    for (let chat of chats) {
                        ...
                        if (!state.existsMap.has(key)) {
                            ...
                            if (!state.record[chat.conversationId]) {
                                commit(chatMutations.INIT_CONVERSATION, {conversation: chat.conversationId})
                            }
                            commit(chatMutations.MODIFY_TEMP_CHAT, {chat});
                            let other = getOtherParticipantID(chat.conversationId, rootState.user.id);
                            let shareKey = getShareKey(rootState.e2ee.publicKeys[other], rootState.e2ee.privateKey, rootState.e2ee.prime);
                            commit(chatMutations.SET_CHAT_SHARE_KEY, {conversation: chat.conversationId, shareKey});
                            chat = await doChatDecrypt(chat, state.shareKeys[chat.conversationId]);
                            commit(chatMutations.ADD_CHAT_RECORD, {chat, isNew});
                            if (isNew) {
                                commit(chatMutations.ADD_CHAT_ADVICE, {chat});
                            }
                            count++;
                            if (chat.status === messageStatusEnums.UNREAD && state.reading && state.reading !== chat.conversationId) {
                                unread++;
        ...
        [chatActions.LOAD_CHAT_PAGE]({commit, state, dispatch}, {conversation}) {
            if (!state.record[conversation]) {
                commit(chatMutations.INIT_CONVERSATION, {conversation})
            }
            return getChatHistory(conversation, state.record[conversation].page).then(res => {
                if (res.code === 200) {
                    let record = res.data.record;
                    dispatch(chatActions.ADD_NEW_CHAT_RECORD, {chats: record, isNew: true});
                    commit(chatMutations.INCREASE_CHAT_PAGE, {conversation, page: 1});
                    commit(chatMutations.SET_CHAT_LOAD_COMPLETE, {conversation, bottom: record.length <= 0});
                } else {
                    throw new Error(res.message);
                }
            });
        ...
        [chatActions.READ_CONVERSATION]({commit, state}, {conversation, status = true}) {
            let unread = state.record[conversation].unread;
            commit(chatMutations.SET_CONVERSATION_UNREAD, {conversation, unread: 0});
